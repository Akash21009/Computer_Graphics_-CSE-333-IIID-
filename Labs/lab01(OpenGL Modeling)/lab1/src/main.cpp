...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glDrawArrays(GL_TRIANGLES, 0, 12*3); // Edit This, (1 face) * (2 triangles per face) * (3 vertices per triangle)
        glUniform3f(vColor_uniform, 0.0, 0.0, 0.0);
        glDrawArrays(GL_LINE_STRIP, 0, 12*3); // Edit This, (1 face) * (2 triangles per face) * (3 vertices per triangle)
...
GLfloat cube_vertices[] = {0, 0, 0,
    ...
    // Indice Order
    GLushort cube_indices[] = {0,1,2, 2, 0, 3,3,6,4,4,0,1,1,5,7,7,2,3,7,6,4,4,5,7,4,3,0,2,7,1,0,1,4,3,6,7};

    int nVertices = 12*3 ; //(1 face) * (2 triangles each) * (3 vertices each)
